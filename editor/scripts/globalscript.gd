...
func _ready():
	...
	if(OS.get_name() == "Windows"):
		OS.set_window_title("dzejmod editor " + VERSION + " (Windows)")
	elif(OS.get_name() == "X11"):
		OS.set_window_title("dzejmod editor " + VERSION + " (Linux)")
	elif(OS.get_name() == "OSX"):
		OS.set_window_title("dzejmod editor " + VERSION + " (MacOS)")
	else:
		OS.set_window_title("dzejmod editor " + VERSION + " (Unknown OS)")
...
func sceneSwtich(resname: String, nomenu: bool = false):
	...
	for node in root.get_children():
		if node != null && !node.name=="console" && !node.name=="pausemenu":
			node.queue_free()
